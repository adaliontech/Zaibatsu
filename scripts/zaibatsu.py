...
from factory_improvement_observation import (
    MAX_IMPROVEMENT_OBSERVATION_BYTES,
    MAX_IMPROVEMENT_OBSERVATION_SPEC_BYTES,
    factory_improvement_observation_for_inputs,
    verify_factory_improvement_observation_for_inputs,
)
from factory_improvement_classification import (
    MAX_IMPROVEMENT_CLASSIFICATION_BYTES,
    MAX_IMPROVEMENT_CLASSIFICATION_POLICY_BYTES,
    factory_improvement_classification_for_inputs,
    verify_factory_improvement_classification_for_inputs,
)
from factory_improvement_candidate import (
    MAX_IMPROVEMENT_CANDIDATE_BYTES,
    MAX_IMPROVEMENT_CANDIDATE_SPEC_BYTES,
    factory_improvement_candidate_for_inputs,
...
from factory_improvement_validation_plan import (
    ...
    MAX_IMPROVEMENT_VALIDATION_PLAN_SPEC_BYTES,
...
def load_improvement_observation_inputs(
    specification_path: str,
    evidence_return_path: str,
    plan_path: str,
    portfolio_path: str,
    ...
    specification = load_bounded_json_document(
        specification_path,
        "factory improvement-observation specification",
        MAX_IMPROVEMENT_OBSERVATION_SPEC_BYTES,
    )
    evidence_return = load_bounded_json_document(
        evidence_return_path,
...
def command_improvement_observation_record(
    specification_path: str,
    evidence_return_path: str,
    plan_path: str,
    portfolio_path: str,
    ...
    inputs = load_improvement_observation_inputs(
        specification_path,
        evidence_return_path,
        plan_path,
        portfolio_path,
    ...
    errors, record = factory_improvement_observation_for_inputs(
        specification,
        evidence_return,
        plan,
        portfolio,
    ...
    if errors or record is None:
        print("cannot build factory improvement-observation record", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    return write_document(record, output)
...
def command_verify_improvement_observation_record(
    record_path: str,
    specification_path: str,
    evidence_return_path: str,
    plan_path: str,
    ...
    record = load_bounded_json_document(
        record_path,
        "factory improvement-observation record",
        MAX_IMPROVEMENT_OBSERVATION_BYTES,
    )
    if record is None:
        return 2
    ...
    errors = verify_factory_improvement_observation_for_inputs(
        record,
        specification,
        evidence_return,
        plan,
    ...
    if errors:
        print(
            f"factory improvement-observation record failed: {record_path}",
            file=sys.stderr,
        )
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    assert isinstance(record, dict)
    print(f"factory improvement-observation record passed: {record_path}")
    print(
        "factory improvement-observation sha256: "
        f"{record['factory_improvement_observation_sha256']}"
    )
    print("observation structurally normalized: true")
    print("observation semantic truth verified: false")
    print("improvement candidate classified: false")
    print("execution authorized: false")
    return 0


def load_improvement_classification_inputs(
    policy_path: str,
    proposal_path: str,
    ...
    qualification_policy_path: str,
    bundle_paths: list[str],
) -> tuple[Any, Any, Any, Any, Any, Any, Any, Any, bytes, Any, Any, list[bytes]] | None:
    if not valid_portfolio_bundle_count(bundle_paths):
        return None
    policy = load_bounded_json_document(
        policy_path,
        "improvement classification policy",
        MAX_IMPROVEMENT_CLASSIFICATION_POLICY_BYTES,
    )
    proposal = load_bounded_json_document(
        proposal_path,
        "factory improvement-proposal record",
        MAX_IMPROVEMENT_PROPOSAL_BYTES,
    )
    observation = load_bounded_json_document(
        observation_path,
        "factory improvement-observation record",
        MAX_IMPROVEMENT_OBSERVATION_BYTES,
    )
    proposal_inputs = load_improvement_proposal_inputs(
    ...
    observation_specification = load_bounded_json_document(
    ...
    if any(
        ...
        return None
    assert proposal_inputs is not None
    (
...
def command_classify_improvement_proposal(
    policy_path: str,
    proposal_path: str,
    proposal_specification_path: str,
    observation_path: str,
    ...
    inputs = load_improvement_classification_inputs(
        policy_path,
        proposal_path,
        proposal_specification_path,
        observation_path,
    ...
    errors, record = factory_improvement_classification_for_inputs(
        policy,
        proposal,
        proposal_specification,
        observation,
    ...
    if errors or record is None:
        print("cannot classify factory improvement proposal", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    return write_document(record, output)
...
def command_verify_improvement_classification(
    record_path: str,
    policy_path: str,
    proposal_path: str,
    proposal_specification_path: str,
    ...
    record = load_bounded_json_document(
        record_path,
        "factory improvement-classification record",
        MAX_IMPROVEMENT_CLASSIFICATION_BYTES,
    )
    if record is None:
        return 2
    ...
    errors = verify_factory_improvement_classification_for_inputs(
        record,
        policy,
        proposal,
        proposal_specification,
    ...
    if errors:
        print(
            f"factory improvement-classification failed: {record_path}",
            file=sys.stderr,
        )
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    assert isinstance(record, dict)
    print(f"factory improvement-classification passed: {record_path}")
    print(
        "factory improvement-classification sha256: "
        f"{record['factory_improvement_classification_sha256']}"
    )
    print(f"decision: {record['classification']['decision']}")
    print(
        "validation planning eligible: "
        f"{str(record['classification']['eligible_for_validation_planning']).lower()}"
    )
    print("content safety scanned: false")
    print("proposal merit verified: false")
    print("promotion authorized: false")
    print("execution authorized: false")
    return 0


def load_improvement_candidate_inputs(
    specification_path: str,
    ...
    qualification_policy_path: str,
    bundle_paths: list[str],
) -> tuple[Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, bytes, Any, Any, list[bytes]] | None:
    if not valid_portfolio_bundle_count(bundle_paths):
        return None
    specification = load_bounded_json_document(
        specification_path,
        "factory improvement-candidate specification",
        MAX_IMPROVEMENT_CANDIDATE_SPEC_BYTES,
    )
    classification = load_bounded_json_document(
        classification_path,
        "factory improvement-classification record",
        MAX_IMPROVEMENT_CLASSIFICATION_BYTES,
    )
    classification_inputs = load_improvement_classification_inputs(
    ...
    if any(value is None for value in (specification, classification, classification_inputs)):
        return None
    assert classification_inputs is not None
    (
...
def command_bind_improvement_candidate(
    specification_path: str,
    classification_path: str,
    classification_policy_path: str,
    proposal_path: str,
    ...
    inputs = load_improvement_candidate_inputs(
        specification_path,
        classification_path,
        classification_policy_path,
        proposal_path,
    ...
    errors, record = factory_improvement_candidate_for_inputs(
        specification,
        classification,
        classification_policy,
        proposal,
    ...
    if errors or record is None:
        print("cannot bind factory improvement candidate", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    return write_document(record, output)
...
def command_verify_improvement_candidate(
    record_path: str,
    specification_path: str,
    classification_path: str,
    classification_policy_path: str,
    ...
    record = load_bounded_json_document(
        record_path,
        "factory improvement-candidate record",
        MAX_IMPROVEMENT_CANDIDATE_BYTES,
    )
    if record is None:
        return 2
    ...
    errors = verify_factory_improvement_candidate_for_inputs(
        record,
        specification,
        classification,
        classification_policy,
    ...
    if errors:
        print(f"factory improvement-candidate failed: {record_path}", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    assert isinstance(record, dict)
    print(f"factory improvement-candidate passed: {record_path}")
    print(
        "factory improvement-candidate sha256: "
        f"{record['factory_improvement_candidate_sha256']}"
    )
    print(f"candidate artifact sha256: {record['candidate']['artifact_sha256']}")
    print("candidate artifact bound: true")
    print("candidate implementation present: false")
    print("validation plan created: false")
    print("promotion authorized: false")
    print("execution authorized: false")
    return 0


def load_improvement_validation_plan_inputs(
    specification_path: str,
    candidate_path: str,
    ...
    specification = load_bounded_json_document(
        specification_path,
        "factory improvement validation-plan specification",
        MAX_IMPROVEMENT_VALIDATION_PLAN_SPEC_BYTES,
    )
    candidate = load_bounded_json_document(
        candidate_path,
        "factory improvement-candidate record",
        MAX_IMPROVEMENT_CANDIDATE_BYTES,
    )
    candidate_inputs = load_improvement_candidate_inputs(
    ...
    if any(value is None for value in (specification, candidate, candidate_inputs)):
        return None
...
def command_plan_improvement_validation(
    specification_path: str,
    candidate_path: str,
    candidate_specification_path: str,
    classification_path: str,
    ...
    errors, record = factory_improvement_validation_plan_for_inputs(
        specification,
        candidate,
        candidate_specification,
        classification,
    ...
    if errors or record is None:
        print("cannot plan factory improvement validation", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    return write_document(record, output)
...
def command_verify_improvement_validation_plan(
    record_path: str,
    specification_path: str,
    candidate_path: str,
    candidate_specification_path: str,
    ...
    errors = verify_factory_improvement_validation_plan_for_inputs(
        record,
        specification,
        candidate,
        candidate_specification,
    ...
    if errors:
        print(
            f"factory improvement validation-plan failed: {record_path}",
            file=sys.stderr,
        )
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    assert isinstance(record, dict)
    print(f"factory improvement validation-plan passed: {record_path}")
    print(
        "factory improvement validation-plan sha256: "
        f"{record['factory_improvement_validation_plan_sha256']}"
    )
    print(f"validation steps: {record['summary']['validation_step_count']}")
    print(f"missing evidence: {record['summary']['missing_evidence_count']}")
    print("candidate implementation bound: false")
    print("ready for validation execution: false")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    improvement_observation_record = commands.add_parser(
        "improvement-observation-record",
        help="structurally normalize one untrusted observation against returned evidence",
    )
    improvement_observation_record.add_argument("specification_path")
    improvement_observation_record.add_argument("evidence_return_path")
    improvement_observation_record.add_argument("plan_path")
    improvement_observation_record.add_argument("portfolio_path")
    improvement_observation_record.add_argument("source_factory_id")
    improvement_observation_record.add_argument("runtime_evidence_pack_path")
    improvement_observation_record.add_argument("qualification_plan_path")
    improvement_observation_record.add_argument("qualification_policy_path")
    improvement_observation_record.add_argument("bundle_paths", nargs="+")
    improvement_observation_record.add_argument(
        "--output",
        help="write the improvement-observation record to a new file",
    )

    verify_improvement_observation_record = commands.add_parser(
        "verify-improvement-observation-record",
        help="reverify a normalized observation and its complete evidence chain",
    )
    verify_improvement_observation_record.add_argument("record_path")
    verify_improvement_observation_record.add_argument("specification_path")
    verify_improvement_observation_record.add_argument("evidence_return_path")
    verify_improvement_observation_record.add_argument("plan_path")
    verify_improvement_observation_record.add_argument("portfolio_path")
    verify_improvement_observation_record.add_argument("source_factory_id")
    verify_improvement_observation_record.add_argument("runtime_evidence_pack_path")
    verify_improvement_observation_record.add_argument("qualification_plan_path")
    verify_improvement_observation_record.add_argument("qualification_policy_path")
    verify_improvement_observation_record.add_argument("bundle_paths", nargs="+")

    classify_improvement_proposal = commands.add_parser(
        "classify-improvement-proposal",
        help="classify a verified proposal and observation for validation planning",
    )
    classify_improvement_proposal.add_argument("policy_path")
    classify_improvement_proposal.add_argument("proposal_path")
    classify_improvement_proposal.add_argument("proposal_specification_path")
    classify_improvement_proposal.add_argument("observation_path")
    classify_improvement_proposal.add_argument("observation_specification_path")
    classify_improvement_proposal.add_argument("evidence_return_path")
    classify_improvement_proposal.add_argument("plan_path")
    classify_improvement_proposal.add_argument("portfolio_path")
    classify_improvement_proposal.add_argument("source_factory_id")
    classify_improvement_proposal.add_argument("runtime_evidence_pack_path")
    classify_improvement_proposal.add_argument("qualification_plan_path")
    classify_improvement_proposal.add_argument("qualification_policy_path")
    classify_improvement_proposal.add_argument("bundle_paths", nargs="+")
    classify_improvement_proposal.add_argument(
        "--output",
        help="write the improvement-classification record to a new file",
    )

    verify_improvement_classification = commands.add_parser(
        "verify-improvement-classification",
        help="reverify a planning-only classification and its complete source chain",
    )
    verify_improvement_classification.add_argument("record_path")
    verify_improvement_classification.add_argument("policy_path")
    verify_improvement_classification.add_argument("proposal_path")
    verify_improvement_classification.add_argument("proposal_specification_path")
    verify_improvement_classification.add_argument("observation_path")
    verify_improvement_classification.add_argument("observation_specification_path")
    verify_improvement_classification.add_argument("evidence_return_path")
    verify_improvement_classification.add_argument("plan_path")
    verify_improvement_classification.add_argument("portfolio_path")
    verify_improvement_classification.add_argument("source_factory_id")
    verify_improvement_classification.add_argument("runtime_evidence_pack_path")
    verify_improvement_classification.add_argument("qualification_plan_path")
    verify_improvement_classification.add_argument("qualification_policy_path")
    verify_improvement_classification.add_argument("bundle_paths", nargs="+")

    bind_improvement_candidate = commands.add_parser(
        "bind-improvement-candidate",
        help="bind one exact non-executable candidate contract to an eligible classification",
    )
    bind_improvement_candidate.add_argument("specification_path")
    bind_improvement_candidate.add_argument("classification_path")
    bind_improvement_candidate.add_argument("classification_policy_path")
    bind_improvement_candidate.add_argument("proposal_path")
    bind_improvement_candidate.add_argument("proposal_specification_path")
    bind_improvement_candidate.add_argument("observation_path")
    bind_improvement_candidate.add_argument("observation_specification_path")
    bind_improvement_candidate.add_argument("evidence_return_path")
    bind_improvement_candidate.add_argument("plan_path")
    bind_improvement_candidate.add_argument("portfolio_path")
    bind_improvement_candidate.add_argument("source_factory_id")
    bind_improvement_candidate.add_argument("runtime_evidence_pack_path")
    bind_improvement_candidate.add_argument("qualification_plan_path")
    bind_improvement_candidate.add_argument("qualification_policy_path")
    bind_improvement_candidate.add_argument("bundle_paths", nargs="+")
    bind_improvement_candidate.add_argument(
        "--output",
        help="write the evidence-bound improvement candidate to a new file",
    )

    verify_improvement_candidate = commands.add_parser(
        "verify-improvement-candidate",
        help="reverify an exact candidate contract and its complete evidence chain",
    )
    verify_improvement_candidate.add_argument("record_path")
    verify_improvement_candidate.add_argument("specification_path")
    verify_improvement_candidate.add_argument("classification_path")
    verify_improvement_candidate.add_argument("classification_policy_path")
    verify_improvement_candidate.add_argument("proposal_path")
    verify_improvement_candidate.add_argument("proposal_specification_path")
    verify_improvement_candidate.add_argument("observation_path")
    verify_improvement_candidate.add_argument("observation_specification_path")
    verify_improvement_candidate.add_argument("evidence_return_path")
    verify_improvement_candidate.add_argument("plan_path")
    verify_improvement_candidate.add_argument("portfolio_path")
    verify_improvement_candidate.add_argument("source_factory_id")
    verify_improvement_candidate.add_argument("runtime_evidence_pack_path")
    verify_improvement_candidate.add_argument("qualification_plan_path")
    verify_improvement_candidate.add_argument("qualification_policy_path")
    verify_improvement_candidate.add_argument("bundle_paths", nargs="+")

    plan_improvement_validation = commands.add_parser(
        "plan-improvement-validation",
        help="bind a deterministic non-executing validation plan to one candidate",
    )
    plan_improvement_validation.add_argument("specification_path")
    plan_improvement_validation.add_argument("candidate_path")
    plan_improvement_validation.add_argument("candidate_specification_path")
    plan_improvement_validation.add_argument("classification_path")
    plan_improvement_validation.add_argument("classification_policy_path")
    plan_improvement_validation.add_argument("proposal_path")
    plan_improvement_validation.add_argument("proposal_specification_path")
    plan_improvement_validation.add_argument("observation_path")
    plan_improvement_validation.add_argument("observation_specification_path")
    plan_improvement_validation.add_argument("evidence_return_path")
    plan_improvement_validation.add_argument("plan_path")
    plan_improvement_validation.add_argument("portfolio_path")
    plan_improvement_validation.add_argument("source_factory_id")
    plan_improvement_validation.add_argument("runtime_evidence_pack_path")
    plan_improvement_validation.add_argument("qualification_plan_path")
    plan_improvement_validation.add_argument("qualification_policy_path")
    plan_improvement_validation.add_argument("bundle_paths", nargs="+")
    plan_improvement_validation.add_argument(
        "--output",
        help="write the deterministic improvement validation plan to a new file",
    )

    verify_improvement_validation_plan = commands.add_parser(
        "verify-improvement-validation-plan",
        help="reverify a non-executing validation plan and its complete evidence chain",
    )
    verify_improvement_validation_plan.add_argument("record_path")
    verify_improvement_validation_plan.add_argument("specification_path")
    verify_improvement_validation_plan.add_argument("candidate_path")
    verify_improvement_validation_plan.add_argument("candidate_specification_path")
    verify_improvement_validation_plan.add_argument("classification_path")
    verify_improvement_validation_plan.add_argument("classification_policy_path")
    verify_improvement_validation_plan.add_argument("proposal_path")
    verify_improvement_validation_plan.add_argument("proposal_specification_path")
    verify_improvement_validation_plan.add_argument("observation_path")
    verify_improvement_validation_plan.add_argument("observation_specification_path")
    verify_improvement_validation_plan.add_argument("evidence_return_path")
    verify_improvement_validation_plan.add_argument("plan_path")
    verify_improvement_validation_plan.add_argument("portfolio_path")
    verify_improvement_validation_plan.add_argument("source_factory_id")
    verify_improvement_validation_plan.add_argument("runtime_evidence_pack_path")
    verify_improvement_validation_plan.add_argument("qualification_plan_path")
...
def main() -> int:
    ...
    if arguments.command == "improvement-observation-record":
        return command_improvement_observation_record(
            arguments.specification_path,
            arguments.evidence_return_path,
            arguments.plan_path,
            arguments.portfolio_path,
    ...
    if arguments.command == "verify-improvement-observation-record":
        return command_verify_improvement_observation_record(
            arguments.record_path,
            arguments.specification_path,
            arguments.evidence_return_path,
            arguments.plan_path,
    ...
    if arguments.command == "classify-improvement-proposal":
        return command_classify_improvement_proposal(
    ...
    if arguments.command == "verify-improvement-classification":
        return command_verify_improvement_classification(
            arguments.record_path,
            arguments.policy_path,
            arguments.proposal_path,
            arguments.proposal_specification_path,
    ...
    if arguments.command == "bind-improvement-candidate":
        return command_bind_improvement_candidate(
            arguments.specification_path,
    ...
    if arguments.command == "verify-improvement-candidate":
        return command_verify_improvement_candidate(
            arguments.record_path,
            arguments.specification_path,
            arguments.classification_path,
            arguments.classification_policy_path,
    ...
    if arguments.command == "plan-improvement-validation":
        return command_plan_improvement_validation(
            arguments.specification_path,
    ...
    if arguments.command == "verify-improvement-validation-plan":
        return command_verify_improvement_validation_plan(
            arguments.record_path,
            arguments.specification_path,
            arguments.candidate_path,
            arguments.candidate_specification_path,
