...
| A proposal drops required review gates, replays another return, or is treated as an accepted improvement | The proposal verifier rechecks the complete evidence-return chain, binds the specification's canonical digest to the exact source and route, restricts targets to shared modules, factory templates, or deterministic gates, requires all later review classes, and fixes authentication, safety, normalization, classification, merit, regression, rollback, approval, promotion, rollout, activation, execution, and cross-factory-effect claims to false |
| An untrusted report is mistaken for a safe or true normalized fact | The observation verifier rechecks the full evidence-return chain and binds only bounded canonical structure, category, subject, narrative, and digest. Reporter authentication, content safety, secret absence, source-artifact truth, report truth, merit, promotion, execution, and cross-factory effects remain false |
| Workflow classification is mistaken for validation, merit, or authority | The separately hashed policy and exact verifier require aligned proposal/observation sources and subjects, allowed types, complete later gates, and non-authorizing inputs. Valid mismatches become deterministic `not_classified` records; forgeries fail. A positive decision permits validation planning only and creates no plan or validation, mutation, approval, promotion, rollout, activation, execution, or cross-factory authority |
| An exact candidate-contract digest is mistaken for safe, correct, implemented, or executable software | Candidate binding reverifies the complete classification and evidence chain, validates bounded canonical structure, requires exact classified-target alignment, and records the exact artifact digest and byte count. The artifact is explicitly untrusted and contract-only; safety, secret absence, semantic truth, implementation, validation planning or execution, regression, rollback, approval, promotion, rollout, activation, execution, and cross-factory effects remain false |
| A deterministic validation plan is mistaken for validation evidence, success, or authority | The planner reverifies the exact candidate chain and only enumerates eight fixed stages plus twelve content-addressed evidence requirements. Every stage is `not_run`, every artifact is `missing`, no implementation exists, all execution counts are zero, and network, production credentials/state, model-as-verifier, overwrite, execution authorization, approval, promotion, rollout, activation, and cross-factory effects are denied |
| A portable validation-input pack is tampered with or mistaken for a runnable, hermetic, approved, or secret-free validation workspace | The bounded USTAR reader rejects traversal, links, special files, duplicate/extra/missing members, metadata drift, trailing bytes, noncanonical JSON, schema substitution, nested-pack or bundle tampering, chain replay, type confusion, and authority inflation. Exact reconstruction proves only portable input identity: the pack omits its verifier runtime and environment, candidate implementation, stage evidence, and every validation, approval, promotion, rollout, activation, execution, and cross-factory authority. Verification needs no live production credential or state, but arbitrary embedded evidence materials are not content-scanned and secret absence remains unproved |
...
- An evidence-bound improvement proposal may still contain malicious,
  sensitive, irrelevant, or ineffective narrative. Its digest proves which
  suggestion was recorded, not that the proposer is authentic or the change
  is safe, useful, classified, tested, approved, or eligible for rollout.
- A content-addressed improvement-candidate contract can still be malicious,
  underspecified, or wrong. Its digest and schema prove which contract was
  bound and how it is shaped, not that an implementation exists or that the
  behavior is safe, correct, testable, regression-free, or promotable.
- A deterministic improvement validation plan can still describe incomplete,
  ineffective, or wrongly scoped future work. Exact reconstruction proves
  which stages and evidence were requested, not that an implementation exists,
  any check ran, evidence is truthful, rollback works, or promotion is safe.
