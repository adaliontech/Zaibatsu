...
| Portable control bundle | Operational in the public kit | Canonical USTAR builder, in-memory verifier, per-file manifest, five bundled schemas, and archive/path/link/metadata/tamper adversarial tests | The same selected control inputs reproduce the same self-contained contract bundle; no runtime implementation, environment realization, deployment, or recovery is included |
