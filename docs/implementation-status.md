...
| Deterministic factory rebuild DAG | Operational in the public kit | Fully reverified bundle/source/qualification inputs, exact nine-action and four-gate rebuild, project-owned schema, and reorder/dependency/replay/shallow-history/type/authority adversarial tests | The kit records the exact intended action order and blockers: 0 of 9 actions are qualification-ready and all 9 remain blocked; current v1 evidence cannot advance them, the graph executes no action, and it grants no qualification, owner approval, activation, deployment, or recovery authority |
