...
| Reusable module catalog | Operational in the public kit | Versioned catalog/schema plus compatibility, dependency, ambiguity, and malformed-input tests | A factory can replace an implementation ID only when the selected module preserves the declared slot policy; bundled entries remain contract-only, source-only, or planned as labeled |
