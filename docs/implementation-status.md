...
| Recursive shared improvement | Return, proposal intake, structural normalization, classification, exact candidate binding, and deterministic non-executing validation planning operational at bounded public-kit scope; broader loop designed | Evidence-return, proposal, observation, classification, candidate-binding, and validation-plan contracts plus lifecycle and promotion contract | Exact untrusted proposals and reports can be bound to verified returned evidence, an aligned candidate contract can be content-addressed, and its required validation work can be enumerated; safety results, semantic validation, implementation, validation execution, promotion, and cross-factory rollout are not deployed |
