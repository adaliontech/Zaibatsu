...
| Recursive shared improvement | Return, proposal intake, structural normalization, validation-planning classification, and exact candidate-contract binding operational at bounded public-kit scope; broader loop designed | Evidence-return, proposal, observation, classification, and candidate-binding contracts plus lifecycle and promotion contract | Exact untrusted proposals and reports can be bound to verified returned evidence, aligned candidates can be classified by workflow/type, and one non-executable candidate contract can be content-addressed against that chain; safety, semantic validation, implementation, validation planning or execution, promotion, and cross-factory rollout are not deployed |
...
| Deterministic improvement-candidate classification | Operational in the public kit | Separately hashed policy and record schemas, full proposal/observation chain re-verification, fixed-order checks, deterministic rejection records, and policy/mismatch/replay/forgery/type/order/CLI adversarial tests | One aligned pair is classified by workflow/type as eligible for validation planning. No validation plan is created, safety or semantic merit decided, test or mutation authorized, owner approval obtained, or promotion, rollout, activation, execution, or cross-factory effect granted |
...
- execution of a validation plan for the classified public improvement
  candidate;
