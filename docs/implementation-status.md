...
| Recursive shared improvement | Return, proposal intake, structural normalization, and validation-planning classification operational at bounded public-kit scope; broader loop designed | Evidence-return, proposal, observation, and classification contracts plus lifecycle and promotion contract | Exact untrusted proposals and reports can be bound to verified returned evidence and aligned candidates can be classified by workflow/type; safety, semantic validation, validation execution, promotion, and cross-factory rollout are not deployed |
