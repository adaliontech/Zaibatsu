...
| --- | --- | --- |
| Architecture validator | v1.9.0 candidate passed locally, from a credential-disabled clone, and in CI; v1.8.0 separately passed immutable tag proof | Current validator requires 87 files and checks 13 components, 3 software factories, 2 portable definitions, 10 catalog modules, bundle/source/qualification/signed-runtime/rebuild contracts and schemas, 11 meta-factory capabilities, 4 sanitized receipts, 9 meta-factory invariants, 8 component invariants, and 10 submission gates. Immutable v1.8.0 remains the latest released baseline until the separate v1.9.0 tag proof completes |
| Adversarial unit tests | v1.9.0 candidate passed locally, from a credential-disabled clone, and in CI; v1.8.0 separately passed immutable tag proof | 194 of 194 current tests passed; immutable v1.8.0 passed its historical 183-test boundary |
...
| Independent secret scan | v1.9.0 candidate passed locally, from a credential-disabled clone, and in CI; v1.8.0 separately passed immutable tag proof | Checksum-pinned Gitleaks `8.30.1` found no leaks in the full public Git history or current tree; the immutable v1.8.0 tag tree passed separately, and GitHub secret scanning and push protection provide independent repository guards |
...
| Demo recording | Pending applicant action | Record from immutable `v1.9.0` using the [Demo script](demo-script.md) after its release proof passes |
